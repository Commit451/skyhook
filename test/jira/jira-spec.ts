...
describe('/POST jira', () => {
    it('issue_updated', async () => {
        const res = await Tester.test(Jira, 'jira-issue.json', null)
    ...
    it('comment_added', async () => {
        const res = await Tester.test(Jira, 'jira-comment.json', null)
        assert.notStrictEqual(res, null)
        assert.ok(Array.isArray(res!.embeds))
        assert.strictEqual(res!.embeds.length, 1)
    })
    ...
    it('custom_no_event', async () => {
        const res = await Tester.test(Jira, 'jira-custom-no-event.json', null)
        assert.ok(!(res instanceof Error))
        assert.strictEqual(res, null)
    })

    it('comment_only', async () => {
        const res = await Tester.test(Jira, 'jira-comment-only.json', null)
        assert.ok(!(res instanceof Error))
        assert.strictEqual(res, null)
    })
    ...
    it('browse_url', async () => {
        ...
        let res = await Tester.testWithBody(Jira, requestBody, null)
        assert.notStrictEqual(res, null)
        ...
        res = await Tester.testWithBody(Jira, requestBody, null)
        assert.notStrictEqual(res, null)
